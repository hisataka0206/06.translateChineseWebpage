...
2. **各子ページを順次処理**
   - タイトルに "done" がある → スキップ（翻訳済み）
   - タイトルに "done" がない → 翻訳処理を実行
...
- **"done" プレフィックス**: 翻訳済みページは自動的にスキップ
- タイトルが `done 人形机器人解析` の場合 → スキップ
- タイトルが `人形机器人解析` の場合 → 翻訳実行
...
**原因1**: タイトルに "done" が含まれている
- **確認**: Notionでページタイトルをチェック
- **解決**: タイトルから "done" を削除
...
### Q4: 翻訳をスキップしたいページがある場合は？

タイトルに "done" を追加すれば自動的にスキップされます。
