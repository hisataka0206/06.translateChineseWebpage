...
class OllamaTranslator:
    """Translator for Chinese to Japanese text using local Ollama instance"""

    def __init__(self, base_url: str = "http://localhost:11434/v1/", model: str = "qwen3:8b", prompt_template: Optional[str] = None):
        """
        Initialize translator with Ollama (via OpenAI SDK)
        ...
        """
        # For Ollama, the API key doesn't matter, but OpenAI SDK requires it
        self.api_key = "ollama"
    ...
    def translate(self, chinese_text: str) -> str:
        """
        Translate Chinese text to Japanese using structured outputs
        ...
        logger.info(f"[Ollama] Translating text: {chinese_text[:50]}...")
        ...
        try:
            ...
            translated_text = response.choices[0].message.parsed.translated_text
            logger.info(f"[Ollama] Translation complete: {translated_text[:50]}...")
    ...
    def generate_title(self, content_snippet: str) -> str:
        """
        Generate a title from content snippet using structured outputs
        ...
        try:
            ...
            logger.info(f"[Ollama] Generated title: {title}")
            return title
