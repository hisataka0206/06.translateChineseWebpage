...
2.  **ブロック解析:** 各ページ内のブロックを走査する。
      ...
      * **画像:** ImageブロックのURLを取得し、一時ファイルとしてダウンロード、またはバイナリとして保持する。
      * *注意:* Notionの画像URLは有効期限があるため、取得直後に処理を行うフローとする。

#### Phase 2: マルチモーダル解析 (Extractor)

1.  **Gemini 1.5 Proへのリクエスト:**
      * 1記事ごとに、「全テキスト」と「全画像データ」をまとめて1回のAPIリクエストとして送信する。
      * **プロンプトの指示内容:**
          * 役割: ロボット工学専門の翻訳家。
          * タスク: テキストおよび画像内の文字から、中国語の専門用語（ヒューマノイド、ハードウェア、AI制御関連）を抽出する。
          * 除外: 一般的な動詞や代名詞。
          * 出力形式: JSONリストのみ。
...
for page in pages:
    ...
    images = notion.download_images(page.id)
...
> 2.  今回は**画像内の文字読み取り精度を重視**するため、LLMには必ず`google-generativeai`ライブラリを使用し、**Gemini 1.5 Pro**モデルを指定してください。
